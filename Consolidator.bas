...
'    '' check the fs date
'    Dim fs_date_origins, fs_date_origin
'    fs_date_origins = Array(Sheets("PL"), Sheets("BS"), Sheets("COSDetail"), Sheets("SalesDetail"))
    fs_date_str = Sheets("PL").Range("A4").Value
'    For Each fs_date_origin In fs_date_origins
'        If fs_date_origin.Range("A4").Value <> fs_date_str Then
'            MsgBox "FS date no match between PL and " & sht.Name
'            End
'        End If
'    Next
...
'' get the names of subsidiaries, including the ones without internal transacitons because the retained earning need to be cancelled
...
FS_order = Array("Instructions", "BS", "PL", "Goodwill", "InternalSales", "COSDetail", "SalesDetail", "Regal Rogue")
...
Cells(margin_row, IDL_col).Value = 0.2
no_IDL:
...
Call calculate_intenal_transaction("COSDetail")
Call calculate_intenal_transaction("SalesDetail")
End Sub
...
If ledger_name = "SalesDetail" Then
    account_name = "40010 - Sales"
ElseIf ledger_name = "COSDetail" Then
    account_name = "50010 - Cost of Goods Sold"
End If
Set name_rng = Cells.Find("Name", searchformat:=False, lookat:=xlPart).EntireColumn
Sheets("InternalSales").Activate
If Cells(1, 2).Value <> "" Then
    account_row = Cells.Find(account_name, searchformat:=False, lookat:=xlPart).Row
...
export_date = extract_wb_date(fs_date_str)
my_path = ThisWorkbook.Path & "\" & export_date & " consolidation.xls"
...
PL_export_path = "F:\Intrepid Spirits\Consolidation\Consolidation Per Brand & Region & Subsidiary\PLperSub" & export_date & ".xlsx"
Sheets("PL").Copy
...
ElseIf date_str Like "*From*" Then '' From Jan yyyy to Apr yyyy
...
Sub aaa()
[B40].FormulaR1C1 = [B40].FormulaR1C1 & "+R[-3]C"
End Sub
